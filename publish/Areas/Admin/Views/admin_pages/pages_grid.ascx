...
<%
    ...
    Html.Telerik().Grid<meridian.bewell.proto.pages>()
            ...
            c =>
                {
                    ...
                    c.Bound(col => col.PageUrl);

                    c.Bound(col => col.id).Edit("admin/admin_pages").Width(100);
                    c.Command(cmd =>
                                  {
                                      cmd.Delete();
                                  }
        ...
        .Sortable( st => st.OrderBy( so => so.Add(e => e.PageUrl)).Enabled(true))
        .Filterable()
