...
{
    public partial class health_factors : BaseEntity, ILookupValueAspectProvider, ILookupValue, IFieldsetAspectProvider, ISeoMetadata, ISeoMetadataAspectProvider
    {
        public override Uri EntityUri()
        {
            string uri = string.Format("/natureheals/healthfactor/{0}/{1}/", id, title.TransliterateAndClear());
            ...
            switch (_fieldName)
            {
                case "parent_id":
                    return new LookupAspect("parent_id", this,
                                            () => Meridian.Default.health_factorsStore.All().Where(s => s.parent_id == 0));
            }
