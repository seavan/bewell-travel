...
{
    public class ResortPhotosAspect : AttachedPhotoAspect
    {
        public ResortPhotosAspect(string field, resorts parent, string folder)
            : base(field, parent, folder)
        {
            _parent = parent;
        }

        private readonly resorts _parent;
        ...
        public override void RemovePhoto(long id)
        {
            ...
            {
                if (File.Exists(HttpContext.Current.Server.MapPath(Path.Combine(Constants.ResortsDataFolder, name))))
                {
                    File.Delete(HttpContext.Current.Server.MapPath(Path.Combine(Constants.ResortsDataFolder, name)));
                }
            }
