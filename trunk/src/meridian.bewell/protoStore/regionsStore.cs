...
namespace meridian.bewell.protoStore
{
	public partial class regionsStore : Meridian.IEntityStore, admin.db.IDataService<proto.regions>
	{
		...
		{
			var cmd = new MySqlCommand("SELECT `id`, `title`, `territory_id`, `health_factors` FROM regions");
		...
		{
			var cmd = new MySqlCommand("INSERT INTO regions ( `title`, `territory_id`, `health_factors` ) VALUES ( @title, @territory_id, @health_factors ); SELECT LAST_INSERT_ID();"); ;
		...
		public proto.regions Update(MySqlConnection _connection, proto.regions _item)
		{
			bool changed =  false;
			var cmdText =  "";
			var cmd = new MySqlCommand("UPDATE regions SET `title`= @title, `territory_id`= @territory_id, `health_factors`= @health_factors WHERE id=@id"); ;
			if(_item.mc_id)
			{
			}
		...
		public void UpdateSync(MySqlConnection _connection, long _id, Meridian _meridian)
		{
			...
			var item = Get(_id);
			var cmd = new MySqlCommand("SELECT `id`, `title`, `territory_id`, `health_factors` FROM regions WHERE id = " + _id.ToString());
			cmd.Connection = _connection;
			using(var reader = cmd.ExecuteReader())
			{
				while(reader.Read())
